...
**Build item #2 is pure integration, not algorithm work.** `analyze_capacity_forecast` and `capacity_calibration`'s `ReleasedForecast` have **zero call sites** in `status_bar.py` (verified by grep); so do `capacity_view.build_capacity_card` / `build_capacity_detail`. Five test files prove the math in isolation. No user has ever seen a burn-rate number from this app.
